  ...
  return (
    <div className="flex-1 overflow-y-auto px-5 pt-12 pb-20">
          ...
          return (
            ...
            >
              <div className="flex items-center space-x-4 mb-3">
                <div className={`w-14 h-14 ${colors.bg} rounded-2xl flex items-center justify-center text-3xl shadow-sm border border-white`}>
                  {studio.icon}
                </div>
              ...
              <div className="pt-2 border-t border-gray-50">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <span className={`text-[9px] font-bold ${isComplete ? 'text-green-600' : colors.text}`}>
                    {Math.round(progress)}%
                  </span>
                </div>
                <div className={`h-1.5 w-full ${colors.bg} rounded-full overflow-hidden shadow-inner`}>
                  <div 
                    className={`h-full transition-all duration-700 ease-out ${isComplete ? 'bg-green-500' : colors.bar}`} 
      ...
      <div 
        ...
        className="mt-8 ios-card p-6 bg-blue-600 text-white ios-tap cursor-pointer relative overflow-hidden group shadow-xl shadow-blue-200"
      >
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white/10 p-3 rounded-xl border border-white/5 backdrop-blur-sm">
            <p className="text-[10px] opacity-70 uppercase font-bold tracking-wider">總品項數</p>
            <p className="text-2xl font-bold">{totalEquipmentCount}</p>
          </div>
          <div className="bg-white/10 p-3 rounded-xl border border-white/5 backdrop-blur-sm">
            <p className="text-[10px] opacity-70 uppercase font-bold tracking-wider">故障/遺失</p>
            <p className="text-2xl font-bold text-orange-300">{totalIssueCount}</p>
          </div>
        </div>
      </div>
    </div>
